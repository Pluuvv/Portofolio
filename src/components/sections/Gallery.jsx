...
import { galleryCategories, photoGallery, videoGallery, graphicGallery, motionGallery } from '../../data/gallery'
    ...
    >
      {!error ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          <span className="text-white/15 text-xs font-medium">Image {index + 1}</span>
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          height: '180px',
          background: 'linear-gradient(160deg, rgba(59,130,246,0.07) 0%, rgba(139,92,246,0.05) 100%)',
      ...
      >
        ...
        <button
          ...
          style={{
            background: item.youtubeId ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.08)',
      ...
      <div className="p-5">
        <h4 className="font-display font-bold text-white/88 text-base mb-1.5">{item.title}</h4>
        ...
        {!item.youtubeId && (
          <p className="text-xs text-white/22 mt-3 italic">Video link not yet added</p>
        )}
      </div>
    </motion.div>
  )
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {!item.image && (
          <p className="text-xs text-white/22 mt-3 italic">Preview image not yet added</p>
        )}
      </div>
    </motion.div>
  )
...
export default function Gallery() {
      ...
      case 'photography':
        return (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
            {photos.map((item, i) => (
              <PhotoItem key={item.id} item={item} index={i} onClick={setLightboxIndex} />
            ))}
          </div>
        )
      case 'videography':
  ...
  return (
    <section id="gallery" className="section-padding relative overflow-hidden" aria-label="Gallery section">
